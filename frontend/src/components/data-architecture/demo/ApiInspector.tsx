...
const ApiLogEntry = React.forwardRef<HTMLDivElement, { log: ApiLog }>(({ log }, ref) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 20 }}
      className="border-l-2 border-teal-500 pl-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
    >
      {/* Log header */}
      <div
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            {/* Request section */}
            <div>
              <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold flex items-center gap-2">
                <span className="text-blue-600 dark:text-blue-400">→</span> REQUEST
              </div>
            ...
            <div>
              <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold flex items-center gap-2">
                <span className="text-green-600 dark:text-green-400">←</span> RESPONSE
              </div>
              <div className="bg-slate-50 dark:bg-slate-900 rounded p-3 border border-slate-200 dark:border-slate-700">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-500 font-mono">
              Transaction: <span className="text-teal-600 dark:text-teal-400">{log.type}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export const ApiInspector: React.FC<ApiInspectorProps> = ({ logs, isLoading, onClear }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-300 dark:border-slate-700 overflow-hidden flex flex-col h-full">
      {/* Header */}
      <div className="bg-slate-100 dark:bg-slate-800 px-4 py-3 border-b border-slate-300 dark:border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Terminal className="w-5 h-5 text-teal-500" />
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">API Inspector</h3>
          {logs.length > 0 && (
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-thin scrollbar-thumb-slate-400 dark:scrollbar-thumb-slate-700 scrollbar-track-slate-100 dark:scrollbar-track-slate-900"
      >
      ...
      {/* Footer with stats */}
      {logs.length > 0 && (
        <div className="bg-slate-100 dark:bg-slate-800 px-4 py-2 border-t border-slate-300 dark:border-slate-700 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <span>
            Success:{' '}
